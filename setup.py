...
VERSION_PATH = os.path.join("moonsilver", "VERSION.txt")
...
def package_data():
    ...
    """
    file_set = []
    for root, dirs, files in os.walk("moonsilver"):
        for f in files:
            if ".git" in f.split(os.path.normpath(os.path.join(root, f))):
                # Prevent the repo from being added.
                continue
            file_name = os.path.relpath(os.path.join(root, f), "moonsilver")
...
setup(
    ...
    author="Volund",
    maintainer="Volund",
    url="https://github.com/volundmush/moonsilver",
    ...
    python_requires=">=3.8",
    project_urls={
        "Source": "https://github.com/volundmush/moonsilver",
        "For": "https://github.com/evennia/honahlee",
        "Issue tracker": "https://github.com/volundmush/moonsilver/issues",
